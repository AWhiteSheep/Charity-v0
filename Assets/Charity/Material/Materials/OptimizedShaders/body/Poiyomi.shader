Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/2898018b4ca12ac48bce39c8a9aaca7e"
{
	Properties
	{
		[HideInInspector] shader_master_label ("<color=#E75898ff>Poiyomi 8.0.293</color>", Float) = 0
...
#define PROP_MATCAP
#define PROP_MATCAPBORDER 0.46
#define PROP_MATCAPMASK
#define PROP_MATCAPMASKUV 0
#define PROP_MATCAPMASKINVERT 0
#define PROP_MATCAPEMISSIONSTRENGTH 0
#define PROP_MATCAPINTENSITY 0.44
#define PROP_MATCAPLIGHTMASK 0
#define PROP_MATCAPREPLACE 0.488
#define PROP_MATCAPMULTIPLY 0.208
#define PROP_MATCAPADD 0.683
...
#define PROP_MATCAP2INTENSITY 1.44
...
#define PROP_EMISSIONSTRENGTH 0.05
...
#define PROP_GITDEMAXLIGHT 0.001
...
#define PROP_GLITTERHUESHIFTENABLED 1
...
#define PROPM_END_STENCILPASSOPTIONS 0
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				switch(float(1))
				{
					case 0:
					{
						...
						matcapUV = noSknewViewNormal.rg * float(0.46) + 0.5;
						break;
					}
					case 1:
					{
						...
						matcapUV = float2(dot(worldViewRight, normal0), dot(worldViewUp, normal0)) * float(0.46) + 0.5;
						break;
					}
					case 2:
					{
				...
				blendMatcap(poiLight, poiFragData, float(0.683), float(0.208), float(0.488), float(0), matcap, matcapMask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
				...
				#if defined(PROP_MATCAP2) || !defined(OPTIMIZER_ENABLED)
				matcap2 = UNITY_SAMPLE_TEX2D_SAMPLER(_Matcap2, _MainTex, TRANSFORM_TEX(matcapUV, _Matcap2)) * float4(poiThemeColor(poiMods, float4(0,0,0,1).rgb, float(0)), float4(0,0,0,1).a);
				#else
				matcap2 = float4(poiThemeColor(poiMods, float4(0,0,0,1).rgb, float(0)), float4(0,0,0,1).a);
				#endif
				matcap2.rgb *= float(1.44);
				#if defined(PROP_MATCAP2MASK) || !defined(OPTIMIZER_ENABLED)
				...
				blendMatcap(poiLight, poiFragData, float(1), float(0), float(1), float(0), matcap2, matcap2Mask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
			...
			{
				float3 emission0 = 0;
				float emissionStrength0 = float(0.05);
				float3 emissionColor0 = 0;
				float glowInTheDarkMultiplier0 = calculateGlowInTheDark(float(0), float(0.001), float(1), float(0), float(1), float(0), poiLight);
				#if defined(PROP_EMISSIONMAP) || !defined(OPTIMIZER_ENABLED)
				if (!float(0))
				{
				...
				#endif
				applyALEmmissionStrength(poiMods, emissionStrength0, float4(0,0,0,0), float(0), float(0));
				applyALCenterOutEmission(poiMods, poiLight.nDotV, emissionStrength0, float(1), float(0), float(0), float4(0,0,0,0), float(0));
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				if (float(0.889))
				{
					rimMask = min(rimMask, lerp(1, poiLight.rampedLightMap, float(0.889)));
				}
				#if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
				...
				float rim = 1 - smoothstep(min(float(0.13), rimWidth), rimWidth, viewDotNormal);
				rim *= float4(0,0,0,1).a * rimColor.a * rimMask;
				poiFragData.emission += rim * lerp(poiFragData.baseColor, rimColor, float(1)) * rimStrength;
				poiFragData.baseColor = lerp(poiFragData.baseColor, lerp(poiFragData.baseColor, rimColor, float(1)) + lerp(poiFragData.baseColor, rimColor, float(1)) * rimBrighten, rim);
			}
			#endif
			#ifdef _RIMSTYLE_UTS2
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			...
			{
				float2 st = frac(poiMesh.uv[float(0)] + float4(0,0,0,0).xy * _Time.x) * float(3000);
				...
				switch(float(0))
				{
					...
					if (float(10) > 0)
					{
						randomRotation = randomFloat3WiggleRange(randoPoint, float(0), float(10));
					}
					else
					{
					...
					finalGlitter = lerp(0, float(0) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, float(0.743)), poiCam.viewDir)), float(1000)), 0);
					...
					finalGlitter = lerp(0, float(0) * glitterAlpha, glitterAlpha) + max(pow(specAngle, float(1000)), 0);
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiLight.lightMap = 1;
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.4735315,0.4072403,0.3564003,1).rgb, float(0));
				poiFragData.alpha = mainTexture.a * float4(0.4735315,0.4072403,0.3564003,1).a;
