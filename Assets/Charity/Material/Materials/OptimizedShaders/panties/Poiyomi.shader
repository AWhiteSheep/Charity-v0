Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/3f8c428dc25aca64da9789ef92eb0412"
{
	Properties
	{
...
#define PROP_MOCHIEREFLECTIONSTRENGTH 0
#define PROP_MOCHIESPECULARSTRENGTH 0.107
...
#define PROPM_END_STENCILPASSOPTIONS 0
			...
			void MochieBRDF(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiLight poiLight, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (float(0.107) > 0)
				{
					...
					specular = poiLight.directColor * specularTerm * fresnelTerm * specularMask * poiThemeColor(poiMods, float4(0,0,0,1), float(0)) * poiLight.occlusion * attenuation * float(0.107);
					#if defined(VERTEXLIGHT_ON) && defined(POI_VERTEXLIGHT_ON)
					for (int index = 0; index < 4; index++)
					{
						fresnelTerm = 1;
						...
						vSpecular += poiLight.vColor[index] * specularTerm * fresnelTerm * specularMask * poiThemeColor(poiMods, float4(0,0,0,1), float(0)) * poiLight.occlusion * poiLight.vAttenuation[index] * float(0.107);
					}
					#endif
				}
				if (float(0) == 1 && float(1) > 0)
				...
				if (float(0) > 0)
				{
					...
					float reflStr = reflectionMask * float(0);
