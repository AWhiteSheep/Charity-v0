Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/d99c1c0ee8733ee4aacedf8b42792b72"
{
	Properties
	{
...
#define PROPM_END_STENCILPASSOPTIONS 0
			...
			void ApplyAlphaToCoverage(inout PoiFragData poiFragData, in PoiMesh poiMesh)
			{
				...
				if (float(1) == 1)
				{
					...
					if (float(0) && float(0))
					{
						poiFragData.alpha *= 1 + max(0, CalcMipLevel(poiMesh.uv[0] * float4(0.0002441406,0.0002441406,4096,4096).zw)) * float(0.25);
			...
			void applyGlitter(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				switch(float(0))
				{
					...
					finalGlitter = lerp(0, float(0) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, float(0)), poiCam.viewDir)), float(1000)), 0);
