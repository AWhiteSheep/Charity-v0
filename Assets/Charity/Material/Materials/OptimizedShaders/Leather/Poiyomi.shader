Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/01a718e9724e70c40bdc2bab9e6293f0"
{
	Properties
	{
...
#define PROP_SHADOWOFFSET -0.462
...
#define PROP_RIMLIGHTCOLORBIAS 0.052
...
#define PROP_MOCHIEROUGHNESSMULTIPLIER 0.431
...
#define PROPM_END_STENCILPASSOPTIONS 0
			...
			void applyShadeMapping(inout PoiFragData poiFragData, PoiMesh poiMesh, inout PoiLight poiLight)
			{
				...
				mainShadowMask *= poiLight.shadowMask * float(0);
				firstSecondShadowMask *= poiLight.shadowMask * float(0);
				if (float(0) == 0)
				{
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				float shadowStrength = float(0) * poiLight.shadowMask;
				#ifdef POI_PASS_OUTLINE
				...
				indirectColor = lerp(poiLight.directColor, indirectColor, float(0) * poiLight.shadowMask);
				...
				float3 mathRamp = lerp(float3(1, 1, 1), saturate(lerp((float4(1,1,1,1) * lerp(indirectColor, 1, float(0))), float3(1, 1, 1), saturate(poiLight.rampedLightMap))), float(0));
				...
				poiLight.finalLighting = finalWrap * saturate(mathRamp + 1 - float(0));
				#endif
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				blendMatcap(poiLight, poiFragData, float(1), float(0.198), float(0), float(0), matcap, matcapMask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				if (float(0))
				{
					rimMask = min(rimMask, lerp(1, poiLight.rampedLightMap, float(0)));
				}
				#if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
				...
				poiFragData.emission += rim * lerp(poiFragData.baseColor, rimColor, float(0.052)) * rimStrength;
				poiFragData.baseColor = lerp(poiFragData.baseColor, lerp(poiFragData.baseColor, rimColor, float(0.052)) + lerp(poiFragData.baseColor, rimColor, float(0.052)) * rimBrighten, rim);
			}
			#endif
			#ifdef _RIMSTYLE_UTS2
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			...
			void MochieBRDF(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiLight poiLight, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (float(1) > 0)
				{
					...
					specular = poiLight.directColor * specularTerm * fresnelTerm * specularMask * poiThemeColor(poiMods, float4(0,0,0,1), float(0)) * poiLight.occlusion * attenuation * float(1);
					#if defined(VERTEXLIGHT_ON) && defined(POI_VERTEXLIGHT_ON)
					for (int index = 0; index < 4; index++)
					{
						fresnelTerm = 1;
						...
						vSpecular += poiLight.vColor[index] * specularTerm * fresnelTerm * specularMask * poiThemeColor(poiMods, float4(0,0,0,1), float(0)) * poiLight.occlusion * poiLight.vAttenuation[index] * float(1);
					}
					#endif
				}
				if (float(0) == 1 && float(1) > 0)
				{
					...
					specular2 = poiLight.directColor * specularTerm * fresnelTerm * specularMask * poiThemeColor(poiMods, float4(0,0,0,1), float(0)) * poiLight.occlusion * attenuation * float(1);
					#if defined(VERTEXLIGHT_ON) && defined(POI_VERTEXLIGHT_ON)
					for (int index = 0; index < 4; index++)
					{
						fresnelTerm = 1;
						...
						vSpecular2 += poiLight.vColor[index] * specularTerm * fresnelTerm * specularMask * poiThemeColor(poiMods, float4(0,0,0,1), float(0)) * poiLight.occlusion * poiLight.vAttenuation[index] * float(1);
					}
					#endif
				}
				if (float(1) > 0)
				{
					...
					reflections *= poiThemeColor(poiMods, float4(1,1,1,1), float(0)) * reflStr;
					#ifdef UNITY_PASS_FORWARDADD
