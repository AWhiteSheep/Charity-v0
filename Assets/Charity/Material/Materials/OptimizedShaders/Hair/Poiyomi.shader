Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/7bbcc1ace87ca8f4b8ae4c5f7572cd27"
{
	Properties
	{
...
#define PROP_MAINBRIGHTNESS -0.107
...
#define PROP_MATCAPINTENSITY 0.45
#define PROP_MATCAPLIGHTMASK 0
#define PROP_MATCAPREPLACE 0.151
...
#define PROP_EMISSIONSTRENGTH 0.74
...
#define PROP_EMISSIONSTRENGTH1 0.05
...
#define PROP_ENABLEGITDEMISSION1 1
...
#define PROP_GITDEMAXLIGHT1 0.001
...
#define PROPM_END_STENCILPASSOPTIONS 0
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				blendMatcap(poiLight, poiFragData, float(0), float(0), float(0.151), float(0), matcap, matcapMask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
			...
			{
				float3 emission0 = 0;
				float emissionStrength0 = float(0.74);
			...
			float3 applyEmission1(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				emissionStrength1 = float(0.05);
				float glowInTheDarkMultiplier1 = calculateGlowInTheDark(float(0), float(0.001), float(1), float(0), float(1), float(0), poiLight);
				#if defined(PROP_EMISSIONMAP1) || !defined(OPTIMIZER_ENABLED)
				if (!float(0))
				{
					emissionColor1 = POI2D_SAMPLER_PAN(_EmissionMap1, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * lerp(1, poiFragData.baseColor, float(1)).rgb * poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0));
				}
				else
				{
					emissionColor1 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap1, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, poiFragData.baseColor, float(1)).rgb * poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0));
				}
				#else
				emissionColor1 = lerp(1, poiFragData.baseColor, float(1)).rgb * poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0));
				#endif
				if (float(0))
				{
				...
				#endif
				applyALEmmissionStrength(poiMods, emissionStrength1, float4(0,0,0,0), float(0), float(0));
				applyALCenterOutEmission(poiMods, poiLight.nDotV, emissionStrength1, float(1), float(0), float(0), float4(0,0,0,0), float(0));
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiLight.lightMap = 1;
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.6903409,0.6903409,0.6903409,1).rgb, float(0));
				poiFragData.alpha = mainTexture.a * float4(0.6903409,0.6903409,0.6903409,1).a;
