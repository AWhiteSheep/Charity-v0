Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/be0a6bdd804ed0648accbfb1d442fb95"
{
	Properties
	{
...
#define PROPM_END_STENCILPASSOPTIONS 0
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
				float4 rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.01219615,0.01219615,0.01219615,1).rgb, float(0)), float4(0.01219615,0.01219615,0.01219615,1).a);
				#else
				float4 rimColor = float4(poiThemeColor(poiMods, float4(0.01219615,0.01219615,0.01219615,1).rgb, float(0)), float4(0.01219615,0.01219615,0.01219615,1).a);
				#endif
				...
				rim *= float4(0.01219615,0.01219615,0.01219615,1).a * rimColor.a * rimMask;
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				float3 rimColor = float3(poiThemeColor(poiMods, float4(0.01219615,0.01219615,0.01219615,1).rgb, float(0)));
